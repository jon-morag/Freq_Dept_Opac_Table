...
function [ kappa_abs_hi_res , kappa_es, nu_calc ] = produce_hires_opac_tbl_rhoT(N_nu,tbl_rho ,tbl_T,include_ff,include_bf,include_bb,A,Z,Xfrac )
...
% parfor i_rho = 1:length(tbl_R) % Run in parallel
    for i_T = 1:length(tbl_T)
        ...
        [ne,Y,e_pop,Zeff] = Saha(tbl_T(i_T)*c.eV,tbl_rho(i_rho),Mode);
