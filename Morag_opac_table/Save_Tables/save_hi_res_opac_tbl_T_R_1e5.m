...
if include_bf_bb
    OpacTableFilename = [MG_opac_dir  mixname 'HiResOpacTableR' num2str(length(tbl_R)) 'T' num2str(length(tbl_T)) 'Nnu1e' num2str(log10(N_nu),2) 'LR' num2str(low_res_spacing) '_es_ff_bf_bb.mat'];
else
    OpacTableFilename = [MG_opac_dir  mixname 'HiResOpacTableR' num2str(length(tbl_R)) 'T' num2str(length(tbl_T)) 'Nnu1e' num2str(log10(N_nu),2) 'LR' num2str(low_res_spacing) '_es_ff.mat'];
end
%%
...
tic
% profile on
[ kappa_abs,kappa_es,nu_calc ] = produce_high_res_opac_tbl_R_T( 1 , N_nu , low_res_spacing, tbl_R , tbl_T ,include_bf_bb, Z_metal); %Z_metal=[]->do nothing
% profile off
toc
...
if include_bf_bb
    ...
    clear kappa_abs
    save(OpacTableFilename , 'kappa_abs_no_bf_bb', 'kappa_es','nu_calc','-v7.3');
end