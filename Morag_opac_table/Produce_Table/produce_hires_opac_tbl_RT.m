...
function [ kappa_abs_hi_res , kappa_es, nu_calc ] = produce_hires_opac_tbl_RT(N_nu,tbl_R ,tbl_T,include_ff,include_bf,include_bb, sobolev, expansion_line_limit, t_sobolev,A,Z,Xfrac )
...
% parfor i_R = 1:length(tbl_R) % Run in parallel
    for i_T = 1:length(tbl_T)
        ...
        [ne,Y,e_pop,Zeff] = SahaEq(tbl_T(i_T)*c.eV,tbl_rho,Mode);
