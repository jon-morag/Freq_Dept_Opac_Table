...
for i_rho = 1:length(tbl_rho)
    for i_T = 1:length(tbl_T)
        for i_nu_lim = 1:length(nu_lims_arr)-1
            ...
            kappa_Ross(i_T,i_rho,i_nu_lim) = int_nu_dim(nu_seg,dBdT_EnergyDistribution(tbl_T(i_T)*c.eV,nu_seg5),2) ./ int_nu_dim(nu_seg,dBdT_EnergyDistribution(tbl_T(i_T)*c.eV,nu_seg5) ./ mid(reshape(kappa_abs(i_T,i_rho,seg_ind) + kappa_es(i_T,i_rho),[1,sum(seg_ind)])) , 2 );
